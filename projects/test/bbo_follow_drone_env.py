...
class BBOFollowDroneEnv(gym.Env):
    """
    ...
    def reset(self, seed=None, options=None):
        ...
        # Spawn balloon at random position
    ...
    def step(self,action):
        ...
        if out_of_bounds:
            ...
            if distance < self.hit_distance:
                ...
                # Small penalty for each step to enourage efficiency
        ...
        # Move balloon randomly (make it harder !)
    ...
    def _move_balloon_randomly(self):
        """ Move balloon slighly in random direction """
    ...
    def _render_frame(self):
        ...
        if self.windows is None and self.render_mode == "human":
            pygame.init()
            pygame.display.init()
            self.windows = pygame.display.set_mode((self.canvas_size, self.canvas_size))
        ...
        pygame.draw.circle(
            canvas, # surface
            (255,0,0), # Red
            self.balloon_pos.astype(int), # Center
            self.balloon_size // 2 # radius
        )
        
        # Draw drone (blue square)
        drone_rect = pygame.Rect(
            self.drone_pos[0] - self.drone_size // 2, # X
            self.drone_pos[1] - self.drone_size // 2, # Y
            self.drone_size, # width
            self.drone_size  # height
        )
        ...
        if self.render_mode == "human":
            self.windows.blit(canvas, canvas.get_rect())
            pygame.event.pump()
...
if __name__ == "__main__":
    env = BBOFollowDroneEnv(render_mode="human")
    obs, info = env.reset()
