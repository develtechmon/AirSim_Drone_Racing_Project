...
REWARD STRUCTURE:
  ...
  +50   = Pass through gate center (clean pass)
  -20   = Pass through gate edge (risky!)
  +50   = Get very close to gate (< 3m)
  +20   = Progress toward gate
  +10-15 = Stay centered when approaching (lateral & vertical)
  -15   = Approach gate from wrong angle
  -50   = Going backwards / wrong direction
  -200  = Crash (STRONG penalty to avoid collisions!)
...
OBSERVATION (25 values):
  ...
  - Lateral offset from gate center (1) ← NEW
  - Vertical offset from gate center (1) ← NEW
  - Approach angle to gate (1) ← NEW
  - Current speed (1) ← NEW
...
class GateRacingEnv(gym.Env):
    ...
    def __init__(self, gate_positions, gate_radius=3.0, render_mode=None):
        ...
        # Observation space (25 values now - added safety info)
        self.observation_space = spaces.Box(
            low=-np.inf,
            high=np.inf,
            shape=(25,),  # Added: lateral offset, vertical offset, gate approach angle, speed
            dtype=np.float32
        )
    ...
    def _get_observation(self):
        ...
        Returns array of 25 values describing current state.
        Includes safety/clearance information to avoid gate collisions.
        """
        state = self.client.getMultirotorState()
        pos = state.kinematics_estimated.position
        ...
        # NEW: Gate alignment metrics (for collision avoidance)
        # Calculate lateral and vertical offset from gate center
        gate_to_drone = drone_pos - current_gate
        
        # Right vector (perpendicular to forward)
        right_vec = np.array([-forward_dir_norm[1], forward_dir_norm[0], 0])
        up_vec = np.array([0, 0, 1])
        
        # How far off-center are we? (lateral = left/right, vertical = up/down)
        lateral_offset = np.dot(gate_to_drone, right_vec)  # Distance from center line
        vertical_offset = np.dot(gate_to_drone, up_vec)    # Height difference
        
        # Approach angle (are we coming straight at gate or from the side?)
        if dist_to_gate > 0.1:
            approach_dir = vec_to_gate / dist_to_gate
            approach_angle = np.arccos(np.clip(np.dot(approach_dir, -forward_dir_norm), -1, 1))
        else:
            approach_angle = 0.0
        ...
        obs = np.concatenate([
            ...
            [lateral_offset],       # 1: NEW - how far left/right from center
            [vertical_offset],      # 1: NEW - how far up/down from center
            [approach_angle],       # 1: NEW - angle of approach to gate
            [speed],                # 1: NEW - current speed magnitude
    ...
    def step(self, action):
        ...
        # Collision check - STRONG PENALTY
        collision_info = self.client.simGetCollisionInfo()
        if collision_info.has_collided:
            terminated = True
            reward -= 200.0  # Increased from 100 to 200 - hitting gates is VERY bad!
    ...
    def _compute_reward(self, old_pos, new_pos, velocity, target_gate, gate_passed, speed, just_passed, steps_since_pass):
        ...
        2. **CENTERING through gates (collision avoidance)**
        3. Getting closer to gates
        4. CONTINUING to next gate after passing current one
        ...
        if gate_passed:
            ...
            # NEW: CENTERING BONUS - Reward passing through center, not edges
            # Calculate how centered the pass was
            gate_to_drone = new_pos - target_gate
            next_idx = (self.current_gate_idx + 1) % self.num_gates
            next_gate = self.gate_positions[next_idx]
            forward_dir = next_gate - target_gate
            forward_norm = forward_dir / (np.linalg.norm(forward_dir) + 1e-6)
            
            right_vec = np.array([-forward_norm[1], forward_norm[0], 0])
            lateral_offset = abs(np.dot(gate_to_drone, right_vec))
            vertical_offset = abs(gate_to_drone[2])
            
            # Bonus for centered pass (within center 50% of gate)
            center_threshold = self.gate_radius * 0.5
            if lateral_offset < center_threshold and vertical_offset < 1.0:
                centering_bonus = 50.0  # Big bonus for clean center pass
            else:
                # Penalty for edge pass (risky!)
                centering_bonus = -20.0
            
            return base_gate_reward + speed_bonus + centering_bonus
        ...
        # NEW: CENTERING REWARD - Reward staying centered when approaching gate
        if new_dist < 8.0:  # Within 8m of gate
            next_idx = (self.current_gate_idx + 1) % self.num_gates
            next_gate = self.gate_positions[next_idx]
            forward_dir = next_gate - target_gate
            forward_norm = forward_dir / (np.linalg.norm(forward_dir) + 1e-6)
            
            # Calculate lateral offset from gate center line
            gate_to_drone = new_pos - target_gate
            right_vec = np.array([-forward_norm[1], forward_norm[0], 0])
            lateral_offset = abs(np.dot(gate_to_drone, right_vec))
            vertical_offset = abs(new_pos[2] - target_gate[2])
            
            # Reward being centered (within ±1.5m laterally, ±1m vertically)
            if lateral_offset < 1.5:
                lateral_centering_reward = (1.5 - lateral_offset) * 10.0
                reward += lateral_centering_reward
            else:
                # Penalty for being off-center when close
                reward -= (lateral_offset - 1.5) * 5.0
            
            if vertical_offset < 1.0:
                vertical_centering_reward = (1.0 - vertical_offset) * 8.0
                reward += vertical_centering_reward
            else:
                # Penalty for wrong altitude when close
                reward -= (vertical_offset - 1.0) * 4.0
            
            # NEW: Approach angle penalty
            # Penalize coming at gate from wrong angle
            approach_vec = new_pos - target_gate
            if np.linalg.norm(approach_vec) > 0.1:
                approach_dir = approach_vec / np.linalg.norm(approach_vec)
                approach_angle = np.arccos(np.clip(np.dot(approach_dir, -forward_norm), -1, 1))
                
                # Penalty for bad approach angle (> 30 degrees)
                if approach_angle > np.pi / 6:  # > 30 degrees
                    angle_penalty = (approach_angle - np.pi / 6) * 15.0
...
if __name__ == "__main__":
    gate_positions = [
        ...
        # [38.8, -4.8, 1.2],  # Gate 3
        # [45.3, 2.5, 1.7],  # Gate 4
        # [45.2, 11.9, 1.4],  # Gate 5
        # [38.1, 19.3, 1.7],  # Gate 6
        # [29.1, 22.4, 1.1],  # Gate 7
        # [17.8, 22.3, 1.6],  # Gate 8
        # [7.2, 18.5, 0.7],  # Gate 9
