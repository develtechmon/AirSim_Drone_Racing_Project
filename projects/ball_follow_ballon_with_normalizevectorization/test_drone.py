...
def test_trained_drone(model_path="drone_ppo_final.zip", num_episodes=5):
    ...
    # ✨ NEW: Wrap in VecEnv for normalization compatibility
    vec_env = DummyVecEnv([lambda: env])
    
    # ✨ NEW: Load the normalization statistics
    print("Loading normalization statistics...")
    vec_env = VecNormalize.load("vec_normalize.pkl", vec_env)
    
    # ✨ NEW: Set to evaluation mode (don't update stats)
    ...
    for episode in range(num_episodes):
        ...
        while True:
            ...
            if done[0]:
                print(f"  Score: {info[0]['score']}/{env.max_score}")
                print(f"  Steps: {steps}")
                print(f"  Total Reward: {episode_reward:.2f}")
                
                if info[0]['score'] >= env.max_score:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--random":
        ...
        # Test trained agent
        test_trained_drone(
            model_path="drone_ppo_final.zip",
            num_episodes=5
        )